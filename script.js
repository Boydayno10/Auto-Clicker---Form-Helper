document.addEventListener('DOMContentLoaded', function() {
     ...
     * @param {string} hash - O hash da URL (ex: '#home').
     */
    function showSection(hash) {
        ...
        if (!targetSectionId || targetSectionId === 'index.html' || targetSectionId === '') {
            targetSectionId = 'home'; // Define "home" como a seção padrão
        }
        ...
        if (legacyMap[targetSectionId]) {
            targetSectionId = legacyMap[targetSectionId];
        }
        ...
        if (targetSection) {
            ...
            // Se a seção não for encontrada (hash inválido), voltar para "home"
        ...
        // Marca como ativo no painel lateral (index)
        const panelLinks = document.querySelectorAll('.config-sidebar a.config-list-item[href^="#"]');
        if (panelLinks.length > 0) {
            panelLinks.forEach(a => a.classList.remove('active'));
            const activePanelLink = document.querySelector(`.config-sidebar a.config-list-item[href="#${targetSectionId}"]`);
            if (activePanelLink) activePanelLink.classList.add('active');
        }

        // Lógica para mostrar/ocultar main-header-content (mantida por compatibilidade)
        if (mainHeaderContent) {
            if (['docs', 'test'].includes(targetSectionId)) {
    ...
    function startGlobalStars() {
        // Se a página não tem o título usado como referência, não inicia loop
        const titleElement = document.querySelector('.header-main-title-desktop');
        if (!titleElement) {
            return;
        }
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function(event) {
            ...
            // Só intercepta navegação quando for um link de âncora (#...
            if (href && href.startsWith('#')) {
                event.preventDefault(); // Impede o comportamento padrão do link
                window.location.hash = href; // Atualiza o hash, que dispara o evento 'hashchange'
            }
            // Caso contrário (ex.: options.html), deixa o navegador seguir o fluxo normal
        });
    });
